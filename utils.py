...
# A dictionnary cntaining all Sboxes
Sboxes = {
...
def expand(r):
    ...
    Args:
        r (string) : A string representing the binary value of R15.
    ...
    '''
    if len(r) != 32:
        raise Exception('The parameter value must be a 32 bits integer. Given one was a ' + str(len(r)) + ' bits parameter.')
...
def exhaustive_attack_sbox(Sbox_name, R15, R15_fault, expected):
    ...
    Args:
        Sbox_name (string) : The name of the Sbox we are attacking.
        R15 (string) : The 6 bits input of the E(R15) value.
        R15_fault (string) : The 6 bits input of the E(R15*) value.
    ...
    '''
    if len(R15) != 6:
        raise Exception('The parameter input_R15 must be a 6 bits integer. Provided was a ' + str(len(R15)) + ' bits value.')
    if len(R15_fault) != 6:
        raise Exception('The parameter input_R15_fault must be a 6 bits integer. Provided was a ' + str(len(R15_fault)) + ' bits value.')
    if len(expected) != 4:
...
def intersect(lists):
    '''
    Returns the intersection of a lists contained in an other list.

    Args:
        lists ((string[])[]) : A list of string lists.
    
    Returns:
        list[string] : The intersection of all sublists.
    '''
    res = lists[0]
